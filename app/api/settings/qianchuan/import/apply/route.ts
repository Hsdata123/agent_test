...
export async function POST(request: Request) {
  try {
    ...
    const originalName = (file.name || "apply.xlsx").replace(/[^A-Za-z0-9._-]/g, "_");
    if (!/\.(xlsx|xls|xlsm)$/i.test(originalName)) {
      throw Object.assign(new Error("仅支持 .xlsx / .xls / .xlsm 文件"), { status: 400 });
    }
    const buffer = Buffer.from(await file.arrayBuffer());
    const tempPath = path.join(os.tmpdir(), `qianchuan-apply-${randomUUID()}-${originalName}`);
    await fs.writeFile(tempPath, buffer);
    try {
      const stdout = await runPython(["apply", tempPath]);
